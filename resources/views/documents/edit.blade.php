    ...
    <div x-data="documentEditor()" @click="closeMenus()" class="flex lg:h-full lg:overflow-hidden">
        ...
        <main class="flex-1 bg-gray-100 min-w-0 flex flex-col lg:overflow-hidden editor-main">
            ...
            <div x-data="{ barZ: false }" x-effect="if (sidebarOpen) { barZ = true } else { setTimeout(() => barZ = false, 200) }"
                 class="bg-white border-b border-gray-200 shadow-sm shrink-0 sticky top-[65px] lg:relative lg:top-0 px-4 h-16 flex items-center editor-topbar" :class="barZ ? 'z-0' : 'z-40'">
                <div class="flex items-center justify-between gap-3 w-full">
                    <div class="flex items-center gap-3 min-w-0">
                        ...
                        @if($meta['id'] ?? null)
                            <span class="text-xs font-mono font-semibold px-1.5 py-0.5 rounded shrink-0 whitespace-nowrap {{ \App\Services\DocumentMetadata::typeColor($meta['type'] ?? '') }}">{{ $meta['id'] }}</span>
                        @endif
