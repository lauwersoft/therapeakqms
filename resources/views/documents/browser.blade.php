        ...
        <div class="flex items-center justify-between">
            ...
            @if($canEdit)
                ...
                <div class="hidden sm:flex items-center gap-2">
                    <button onclick="window.dispatchEvent(new CustomEvent('start-bulk-export'))" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-red-600 text-white rounded-md text-sm hover:bg-red-700 whitespace-nowrap">
                    ...
                    <button onclick="document.getElementById('browse-upload-modal-toggle').click()" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white border border-gray-300 rounded-md text-sm text-gray-600 hover:bg-gray-50 whitespace-nowrap">
                    ...
                    <button @click="ctx.dir = ''; quickCreateModal = true; $nextTick(() => $refs.browseQuickCreateInput?.focus())" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 text-white text-sm rounded-md hover:bg-blue-700 whitespace-nowrap">
                ...
                {{-- Mobile menu --}}
                <div class="sm:hidden relative" x-data="{ menuOpen: false }">
                    <button @click="menuOpen = !menuOpen" class="p-2 rounded-md hover:bg-gray-100 text-gray-500">
                    ...
                    <div x-show="menuOpen" x-cloak @click.outside="menuOpen = false"
                        ...
                        <button @click="menuOpen = false; ctx.dir = ''; quickCreateModal = true; $nextTick(() => $refs.browseQuickCreateInput?.focus())" class="flex items-center gap-2 w-full px-3 py-2 text-sm text-gray-700 hover:bg-gray-100">
                        ...
                        <button @click="menuOpen = false; document.getElementById('browse-upload-modal-toggle').click()" class="flex items-center gap-2 w-full px-3 py-2 text-sm text-gray-700 hover:bg-gray-100">
            ...
            <div x-show="ctx.show" x-cloak
                ...
                <template x-if="ctx.type === 'file'">
                    <div>
                        ...
                        <a :href="ctx.isMarkdown ? ('/documents/edit/' + ctx.path) : ('/documents/download/' + ctx.path)"
                                    ...
                                    <a :href="'/documents/' + doc.url_path"
                                       ...
                                       class="block px-4 sm:px-5 py-3 hover:bg-gray-50 transition-colors border-b border-gray-50 last:border-b-0">
                                        ...
                                        <div class="hidden lg:flex items-center gap-4">
                                            ...
                                            <span class="shrink-0 text-[11px] font-medium px-1.5 py-0.5 rounded" :class="statusClass(doc.status)" x-text="doc.status_label"></span>
                                            ...
                                            <span x-show="doc.changed" class="w-2 h-2 rounded-full shrink-0" :class="{'bg-green-500': doc.changed === 'new' || doc.changed === 'added', 'bg-amber-500': doc.changed === 'modified', 'bg-red-500': doc.changed === 'deleted', 'bg-blue-500': doc.changed === 'move' || doc.changed === 'rename'}"></span>
                                            <template x-if="doc.is_markdown || doc.path.endsWith('.form.json')">
                                                <a :href="doc.is_markdown ? ('/documents/export/' + doc.path) : ('/documents/export-form/' + doc.path)" @click.stop class="p-1 rounded bg-red-50 text-red-400 hover:bg-red-100 hover:text-red-600 shrink-0 transition-colors" title="Download PDF">
                                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                                </a>
                                            </template>
                                            <template x-if="doc.is_markdown && doc.has_tables">
                                                <a :href="'/documents/export-xlsx/' + doc.path" @click.stop class="p-1 rounded bg-green-50 text-green-400 hover:bg-green-100 hover:text-green-600 shrink-0 transition-colors" title="Download XLSX">
                                            ...
                                            <template x-if="!doc.is_markdown && !doc.path.endsWith('.form.json')">
                                                <a :href="'/documents/download/' + doc.path" @click.stop class="p-1 rounded bg-gray-100 text-gray-400 hover:bg-gray-200 hover:text-gray-600 shrink-0 transition-colors" title="Download file">
