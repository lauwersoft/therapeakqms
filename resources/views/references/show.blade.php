                ...
                <div class="flex-1 overflow-y-auto overscroll-contain" x-ref="tocContainer">
                    <nav class="px-2 py-2">
                        @foreach($toc as $idx => $item)
                            <a href="#{{ $item['id'] }}" @click.prevent="sidebarOpen = false; var el = document.getElementById('{{ $item['id'] }}'); if(el){ if(window.innerWidth<1024){ window.scrollTo({top: el.getBoundingClientRect().top + window.scrollY - 140}); } else { var c=$refs.content; c.scrollTop = el.offsetTop - c.offsetTop - 20; } highlightRefSection('{{ $item['id'] }}'); }"
                               :id="'toc-' + '{{ $item['id'] }}'"
