...
| ID | Requirement | Rationale |
|---|---|---|
| PF-001 | The system shall target 99.9% availability, achieved through multi-provider infrastructure redundancy (requests routed via OpenRouter gateway through Vertex AI, Amazon Bedrock, and Anthropic API). | Ensure reliable access to therapy services. |
...
| ID | Requirement | Rationale |
|---|---|---|
| AI-001 | The primary therapy AI model shall be Anthropic Claude Sonnet 4.5, with multi-layer fallback: Claude Opus 4.5, then Claude Sonnet 4, Claude Sonnet 3.7, Claude Opus 4. | Ensure continuous AI availability through redundancy. |
| AI-002 | API requests to Anthropic shall be routed via the OpenRouter gateway, which provides infrastructure-level redundancy through Vertex AI, Amazon Bedrock, and the Anthropic API. | Eliminate single-infrastructure-provider dependency. |
...
| AI-007 | Data sharing at the OpenRouter gateway shall be disabled to prevent therapy conversation data from being used for third-party training. | Protect user health data privacy. |
