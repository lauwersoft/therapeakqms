...
class GitService
{
    ...
    public function getHistory(int $limit = 50, int $offset = 0): array
    {
        ...
        $result = Process::path($this->base)
            ->run(['git', 'log', '--no-merges', '--after=2026-03-26T00:00:00+01:00', '--format=%H|%an|%ae|%aI|%s', '-' . $fetchLimit, '--', 'qms/documents/']);
    ...
    {
        $result = Process::path($this->base)
            ->run(['git', 'log', '--no-merges', '--after=2026-03-26T00:00:00+01:00', '--oneline', '--', 'qms/documents/']);
    ...
    {
        $result = Process::path($this->base)
            ->run(['git', 'log', '--no-merges', '--after=2026-03-26T00:00:00+01:00', '--format=%H|%an|%ae|%aI|%s', '-' . $limit, '--', 'qms/documents/' . $path]);
